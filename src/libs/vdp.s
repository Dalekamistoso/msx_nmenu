...
; System calls
	...
	CALSLT = 0x001c    ; Executes inter-slot call [Input: IY-High byte with slot ID | IX-The address that will be called]
...
; System variables
	CLIKSW  = 0xf3db   ; (BYTE) Key click switch. (0 = Disabled / 1 = Enabled)
	RG0SAV  = 0xf3df   ; (BYTE) Mirror of VDP Register 0 (R#0)
	FORCLR	= 0xf3e9   ; (BYTE) Foreground colour
	BAKCLR	= 0xf3ea   ; (BYTE) Background colour
	BDRCLR	= 0xf3eb   ; (BYTE) Border colour
	DPPAGE  = 0xfaf5   ; (BYTE) Display Page Number
	ACPAGE  = 0xfaf6   ; (BYTE) Active Page Number
	EXPTBL	= 0xfcc1   ; (BYTE) BIOS slot
	RG8SAV  = 0xffe7   ; (BYTE) Mirror Of VDP Register 8 (R#8)
